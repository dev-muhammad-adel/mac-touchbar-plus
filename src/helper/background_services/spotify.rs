...
async fn execute_seek_with_ui_update(
    position: f64, 
...
// Old function removed - we now use set_current_mpris_service directly

// Set MPRIS service directly by name - no window class detection needed
pub fn set_current_mpris_service(mpris_name: &str) {
    log_info(&format!("set_current_mpris_service called with MPRIS name: '{}'", mpris_name));
    
    // Create instance with the provided MPRIS name
    let instance = MediaPlayerInstance::new(mpris_name.to_string(), "generic".to_string(), None);
    
    // Update the global instance and state
    unsafe {
        CURRENT_MEDIA_PLAYER_INSTANCE = Some(instance.clone());
        IS_USING_INSTANCE = false; // We're using the exact MPRIS name, not an instance
        log_info(&format!("Set current MPRIS service to: {}", mpris_name));
    }
}
...
pub fn get_spotify_status() -> Option<MediaStatus> {
    ...
    tokio::task::block_in_place(|| {
        ...
        log_error("MPRIS", "connection failed");
        None
    })
}
...
pub async fn get_spotify_status_async() -> Option<MediaStatus> {
    ...
    log_error("MPRIS (async)", "connection failed");
    None
}

async fn execute_spotify_command(command: &str, args: &[&str]) -> bool {
    let mpris_dest = get_current_mpris_destination();
    log_info(&format!("Executing MPRIS command '{}' on {}", command, mpris_dest));
    
    // Clone the data to move into the closure
    let command = command.to_string();
    let args: Vec<String> = args.iter().map(|s| s.to_string()).collect();
    let mpris_dest = mpris_dest;
    
    // Use spawn_blocking to avoid runtime conflicts
    tokio::task::spawn_blocking(move || {
        let args_refs: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
        TOKIO_RT.block_on(try_execute_command_on_destination_native(&command, &args_refs, &mpris_dest))
    }).await.unwrap_or(false)
}

// MPRIS service management - no longer need window class checking

pub async fn handle_spotify_command(command: &str, status_sender: &Arc<Mutex<Option<UnixStream>>>) {
    ...
    match command.trim() {
        "play_pause" => {
            ...
            execute_spotify_command("org.mpris.MediaPlayer2.Player.PlayPause", &[]).await;
            send_status_if_available(status_sender);
        }
        "play" => {
            log_info("Executing play command");
            execute_spotify_command("org.mpris.MediaPlayer2.Player.Play", &[]).await;
            send_status_if_available(status_sender);
        }
        "pause" => {
            log_info("Executing pause command");
            execute_spotify_command("org.mpris.MediaPlayer2.Player.Pause", &[]).await;
            send_status_if_available(status_sender);
        }
        "next" => {
            log_info("Executing next command");
            execute_spotify_command("org.mpris.MediaPlayer2.Player.Next", &[]).await;
        }
        "previous" => {
            log_info("Executing previous command");
            execute_spotify_command("org.mpris.MediaPlayer2.Player.Previous", &[]).await;
        }
        "stop" => {
            log_info("Executing stop command");
            execute_spotify_command("org.mpris.MediaPlayer2.Player.Stop", &[]).await;
            send_status_if_available(status_sender);
        }
        "raise" => {
            log_info("Executing raise command");
            execute_spotify_command("org.mpris.MediaPlayer2.Raise", &[]).await;
        }
        "quit" => {
            log_info("Executing quit command");
            execute_spotify_command("org.mpris.MediaPlayer2.Quit", &[]).await;
        }
            ...
            if let Some(position_str) = cmd.strip_prefix("seek:") {
                if let Ok(mut position) = position_str.parse::<f64>() {
                    ...
                    if can_seek {
                        ...
                        execute_seek_with_ui_update(position, status_sender, "Seeking").await;
                    } else {
        ...
        cmd if cmd.starts_with("set_position:") => {
            if let Some(position_str) = cmd.strip_prefix("set_position:") {
                if let Ok(mut position) = position_str.parse::<f64>() {
                    ...
                    execute_seek_with_ui_update(position, status_sender, "Set position").await;
                } else {
