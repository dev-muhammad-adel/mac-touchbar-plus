...
// Generic Media Helper Manager - Similar to MediaPlayerHelperManager but only for Spotify
pub struct GenericMediaHelperManager {
    process: Option<Child>,
    listener: Option<UnixListener>,
    process_info: ProcessInfo,
    auto_restart_enabled: bool,
...
impl GenericMediaHelperManager {
    pub fn new() -> Self {
        GenericMediaHelperManager {
            process: None,
            listener: None,
            process_info: ProcessInfo::new(),
            auto_restart_enabled: true,
            socket_path: "/tmp/touchbar-generic-media.sock".to_string(),
            window_class: None,
            window_id: None,
            pid: None,
        }
    ...
    pub fn start(&mut self, user: &str, leader_pid: u32, window_class: &str, window_id: u64, pid: u32) -> Option<i32> {
        ...
        let listener = match UnixListener::bind(socket_path) {
            Ok(listener) => listener,
            Err(e) => {
                println!("[GenericMediaHelperManager::start] ERROR: Failed to bind Generic Media socket: {}", e);
                self.process_info.status = ProcessStatus::Failed;
                self.process_info.consecutive_failures += 1;
                return None;
            }
        ...
        if let Err(e) = listener.set_nonblocking(true) {
            println!("[GenericMediaHelperManager::start] ERROR: Failed to set Generic Media socket non-blocking: {}", e);
            self.process_info.status = ProcessStatus::Failed;
            self.process_info.consecutive_failures += 1;
            return None;
        }
        ...
        let userinfo = match User::from_name(user) {
            Ok(Some(user)) => user,
            Ok(None) => {
                println!("[GenericMediaHelperManager::start] ERROR: User '{}' not found", user);
                self.process_info.status = ProcessStatus::Failed;
                self.process_info.consecutive_failures += 1;
                return None;
            }
            Err(e) => {
                println!("[GenericMediaHelperManager::start] ERROR: Failed to get user info for '{}': {}", user, e);
                self.process_info.status = ProcessStatus::Failed;
                self.process_info.consecutive_failures += 1;
                return None;
            }
        ...
        // Set socket ownership to the user
        if let Err(e) = chown(socket_path.as_str(), Some(userinfo.uid), Some(userinfo.gid)) {
            println!("[GenericMediaHelperManager::start] WARNING: Failed to set socket ownership: {}", e);
        }

        // Get environment variables from the session leader
        let mut env = get_env_from_pid(leader_pid);
        env.extend(get_env_from_session(user, leader_pid));

        // Set specific environment variables for the helper
        env.insert("TINY_DFR_WINDOW_CLASS".to_string(), window_class.to_string());
        env.insert("TINY_DFR_WINDOW_ID".to_string(), window_id.to_string());
        env.insert("TINY_DFR_WINDOW_PID".to_string(), pid.to_string());

        // Start the generic media helper process
        let mut cmd = Command::new("sudo");
        cmd.args(&["-u", user, "/usr/bin/tiny-dfr-generic-media-helper"]);
        cmd.env_clear();
        cmd.envs(&env);
        ...
        match cmd.spawn() {
            Ok(child) => {
                println!("[GenericMediaHelperManager::start] Started Generic Media helper process with PID: {}", child.id());
                self.process = Some(child);
                self.listener = Some(listener);
                self.window_class = Some(window_class.to_string());
                self.window_id = Some(window_id);
            ...
            Err(e) => {
                println!("[GenericMediaHelperManager::start] ERROR: Failed to start Generic Media helper process: {}", e);
                self.process_info.status = ProcessStatus::Failed;
                self.process_info.consecutive_failures += 1;
                return None;
            }
        ...
        println!("[GenericMediaHelperManager::start] Generic Media helper started successfully, listening on fd: {}", fd);
    ...
    pub fn stop(&mut self) {
        if let Some(mut child) = self.process.take() {
            println!("[GenericMediaHelperManager::stop] Stopping Generic Media helper process with PID: {}", child.id());
            ...
            if result != 0 {
                ...
                // Only log error if it's not "No such process" (process already dead)
                if errno.raw_os_error() != Some(3) { // ESRCH = 3
                    println!("[GenericMediaHelperManager::stop] WARNING: Failed to send SIGTERM: {}", errno);
                }
            }
    ...
    /// Check if the Generic Media helper process is still running and clean up zombies
    pub fn check_process_status(&mut self) -> bool {
        let now = Instant::now();
        
        // Only check if we have a process
        if let Some(ref mut child) = self.process {
            // Check if process is still running
            match child.try_wait() {
                Ok(Some(status)) => {
                    println!("[GenericMediaHelperManager::check_process_status] Generic Media helper process exited with status: {:?}", status);
                    self.process = None;
                    self.listener = None;
                    self.process_info.status = ProcessStatus::Stopped;
                    return false;
                ...
                Err(e) => {
                    println!("[GenericMediaHelperManager::check_process_status] ERROR: Failed to check Generic Media helper process status: {}", e);
                    self.process = None;
                    self.listener = None;
                    self.process_info.status = ProcessStatus::Failed;
                    return false;
    ...
    pub fn accept_connection(&mut self) -> Option<UnixStream> {
        if let Some(ref listener) = self.listener {
            match listener.accept() {
                Ok((stream, _)) => {
                    println!("[GenericMediaHelperManager::accept_connection] Accepted Generic Media helper connection");
                    Some(stream)
                }
                ...
                Err(e) => {
                    println!("[GenericMediaHelperManager::accept_connection] ERROR: Failed to accept Generic Media helper connection: {}", e);
                    None
                }
            }
        } else {
    ...
    pub fn force_cleanup(&mut self) {
        if let Some(mut child) = self.process.take() {
            println!("[GenericMediaHelperManager::force_cleanup] Force cleaning up Generic Media helper process with PID: {}", child.id());
            let _ = child.kill();
            let _ = child.wait();
        }
        self.listener.take();
