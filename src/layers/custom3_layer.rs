...
type Custom3LayerResult<T> = Result<T, Custom3LayerError>;
...
pub struct Custom3Layer {
    pub buttons: Vec<Button>,
}

impl Custom3Layer {
    /// Creates a custom3 layer with buttons
    pub fn with_config(cfg: Vec<ButtonConfig>) -> Custom3Layer {
        if cfg.is_empty() {
            panic!("Invalid configuration, Custom3 layer has 0 buttons");
        }
        Custom3Layer {
    ...
    fn safe_cairo_context(surface: &Surface) -> Custom3LayerResult<Context> {
        Context::new(surface)
            .map_err(|e| Custom3LayerError::Cairo(format!("Failed to create Cairo context: {}", e)))
    }

    fn safe_cairo_paint(c: &Context) -> Custom3LayerResult<()> {
        c.paint()
            .map_err(|e| Custom3LayerError::Cairo(format!("Failed to paint: {}", e)))
    }

    fn safe_cairo_fill(c: &Context) -> Custom3LayerResult<()> {
        c.fill()
            .map_err(|e| Custom3LayerError::Cairo(format!("Failed to fill: {}", e)))
    }

    // Helper function to setup font rendering
    fn setup_font_rendering(c: &Context, config: &Config) -> Custom3LayerResult<()> {
        match config.font_renderer.to_lowercase().as_str() {
            ...
            renderer => Err(Custom3LayerError::InvalidFontRenderer(renderer.to_string()))
        }?;
    ...
    fn draw_rounded_button_static(c: &Context, left_edge: f64, bot: f64, top: f64, width: f64, radius: f64) -> Custom3LayerResult<()> {
        c.new_sub_path();
...
impl Layer for Custom3Layer {
    fn draw(
        ...
        let gap = APP_LAYER_KEYS3_GAP_PX;
        ...
        if complete_redraw {
            // Set black background for Custom3Layer
            c.set_source_rgb(0.0, 0.0, 0.0);
            c.paint().map_err(|e| Custom3LayerError::Cairo(format!("Failed to paint black background: {}", e)))?;
        }
        ...
        for (i, button) in self.buttons.iter_mut().enumerate() {
            ...
            if !complete_redraw {
                // Account for slide animation offset in clip rect coordinates
                let clip_rect = ClipRect::new(
                    height as u16 - top as u16 - BUTTON_RADIUS as u16,
                    (left_edge + slide_offset) as u16,
