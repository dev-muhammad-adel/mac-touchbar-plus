...
type Custom2LayerResult<T> = Result<T, Custom2LayerError>;
...
pub struct Custom2Layer {
    pub buttons: Vec<Button>,
}

impl Custom2Layer {
    /// Creates a custom2 layer with buttons
    pub fn with_config(cfg: Vec<ButtonConfig>) -> Custom2Layer {
        if cfg.is_empty() {
            panic!("Invalid configuration, Custom2 layer has 0 buttons");
        }
        Custom2Layer {
    ...
    fn safe_cairo_context(surface: &Surface) -> Custom2LayerResult<Context> {
        Context::new(surface)
            .map_err(|e| Custom2LayerError::Cairo(format!("Failed to create Cairo context: {}", e)))
    }

    fn safe_cairo_paint(c: &Context) -> Custom2LayerResult<()> {
        c.paint()
            .map_err(|e| Custom2LayerError::Cairo(format!("Failed to paint: {}", e)))
    }

    fn safe_cairo_fill(c: &Context) -> Custom2LayerResult<()> {
        c.fill()
            .map_err(|e| Custom2LayerError::Cairo(format!("Failed to fill: {}", e)))
    }

    // Helper function to setup font rendering
    fn setup_font_rendering(c: &Context, config: &Config) -> Custom2LayerResult<()> {
        match config.font_renderer.to_lowercase().as_str() {
            ...
            renderer => Err(Custom2LayerError::InvalidFontRenderer(renderer.to_string()))
        }?;
    ...
    fn draw_rounded_button_static(c: &Context, left_edge: f64, bot: f64, top: f64, width: f64, radius: f64) -> Custom2LayerResult<()> {
        c.new_sub_path();
...
impl Layer for Custom2Layer {
    fn draw(
        ...
        let gap = APP_LAYER_KEYS2_GAP_PX;
