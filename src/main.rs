...
use crate::services::sessionmanager::{monitor_sessions, SessionState};
use chrono::{Local, Timelike};
use tokio::sync::{mpsc, watch};
use view::app_ui_manager::{AppAction, AppUiManager};
...
fn safe_cairo_render_document(
    renderer: &CairoRenderer,
    c: &Context,
    rect: &Rectangle,
) -> MainResult<()> {
    renderer
        .render_document(c, rect)
...
fn safe_drm_map(drm: &mut DrmBackend) -> MainResult<DumbMapping> {
    drm.map().map_err(|e| MainError::Drm(e.into()))
}

fn safe_drm_dirty(drm: &mut DrmBackend, clips: &[ClipRect]) -> MainResult<()> {
    drm.dirty(clips).map_err(|e| MainError::Drm(e.into()))
}

fn safe_stream_try_clone(stream: &UnixStream) -> MainResult<UnixStream> {
    stream.try_clone().map_err(|e| MainError::Input(e))
}
...
fn safe_epoll_wait(epoll: &Epoll, events: &mut [EpollEvent], timeout: isize) -> MainResult<usize> {
    epoll.wait(events, timeout).map_err(|e| MainError::Epoll(e))
}

fn safe_input_dispatch(input: &mut Libinput) -> MainResult<()> {
    input.dispatch().map_err(|e| MainError::Input(e))
}

// Helper function for safe layer access
fn get_active_layer_mut(
    layers: &mut HashMap<LayerKey, FunctionLayer>,
    active_layer: LayerKey,
) -> MainResult<&mut FunctionLayer> {
    layers
        .get_mut(&active_layer)
        .ok_or_else(|| MainError::LayerNotFound(active_layer))
}

fn get_active_layer(
    layers: &HashMap<LayerKey, FunctionLayer>,
    active_layer: LayerKey,
) -> MainResult<&FunctionLayer> {
    layers
        .get(&active_layer)
        .ok_or_else(|| MainError::LayerNotFound(active_layer))
}
...
fn get_touch_slot<'a>(
    ...
    slot: u32,
) -> MainResult<&'a (LayerKey, &'static str, usize)> {
    touches
        .get(&slot)
...
fn get_pending_layer(pending_layer: &mut Option<LayerKey>) -> MainResult<LayerKey> {
    pending_layer.take().ok_or(MainError::PendingLayerNotFound)
}
...
use crate::helper::manager::{BrowserHelperManager, HelperManager, VlcHelperManager};
...
impl FunctionLayer {
    ...
    fn draw(
        ...
        mut app_ui_manager: Option<&mut AppUiManager>,
        vlc_drag_position: Option<f64>,
    ) -> MainResult<Vec<ClipRect>> {
        match &mut self.split {
            Some(split) => {
                ...
                let _media_spacing = if media_count > 1 {
                    BUTTON_SPACING_PX as f64 * (media_count as f64 - 1.0)
                } else {
                    0.0
                };
                ...
                let total_spacing = if media_count > 1 {
                    media_spacing_px * (media_count as f64 - 1.0)
                } else {
                    0.0
                };
                let button_area = media_width - total_spacing;
                let weights: Vec<f32> = split
                    .media
                    .iter()
                    .map(|b| b.fraction.unwrap_or(1.0))
                    .collect();
                let total_weight: f32 = weights.iter().sum();
                let mut media_button_widths: Vec<f64> = weights
                    .iter()
                    .map(|w| button_area * (*w as f64 / total_weight as f64))
                    .collect();
                ...
                if config.font_renderer.to_lowercase() == "cairo" {
                    c.select_font_face(
                        ...
                        if config.italic_cairo {
                            FontSlant::Italic
                        } else {
                            FontSlant::Normal
                        },
                        if config.bold_cairo {
                            FontWeight::Bold
                        } else {
                            FontWeight::Normal
                        },
                    ...
                    panic!(
                        "Invalid font renderer chosen. Choose between \"Cairo\" and \"FreeType\""
                    );
                }
                ...
                let left_edge =
                    pixel_shift_x + (pixel_shift_width / 2) as f64 + modules_width + group_spacing;
            ...
            None => {
                ...
                if let Some(LayerKey::Custom3) = layer_index {
                    ...
                    let slide_offset = if app_layer3_slide_progress < 1.0 {
                        let direction = if app_layer3_slide_progress > 0.0 {
                            1.0
                        } else {
                            -1.0
                        };
                ...
                let weights: Vec<f32> = self
                    ...
                    .collect();
                let total_weight: f32 = weights.iter().sum();
                let mut button_widths: Vec<f64> = weights
                    .iter()
                    .map(|w| button_area * (*w as f64 / total_weight as f64))
                    .collect();
                // Last button absorbs rounding error
...
pub fn toggle_key<F>(uinput: &mut UInputHandle<F>, code: Key, value: i32)
where
    F: AsRawFd,
{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    rt.block_on(async { real_main(&mut drm).await })?;
...
async fn real_main(drm: &mut DrmBackend) -> MainResult<()> {
    ...
    let fb_info = drm.fb_info().map_err(|e| MainError::Drm(e.into()))?;
    ...
    let mut backlight = BacklightManager::new().map_err(|e| {
        MainError::Display(format!("Failed to initialize backlight manager: {}", e))
    })?;
    ...
    input_tb.udev_assign_seat("seat-touchbar").map_err(|_| {
        MainError::Input(std::io::Error::new(
            std::io::ErrorKind::Other,
            "Failed to assign touch bar seat",
        ))
    })?;
    input_main.udev_assign_seat("seat0").map_err(|_| {
        MainError::Input(std::io::Error::new(
            std::io::ErrorKind::Other,
            "Failed to assign main seat",
        ))
    })?;

    // Safe epoll creation
    let epoll = Epoll::new(EpollCreateFlags::empty()).map_err(|e| MainError::Epoll(e))?;
    ...
    // --- eventfd integration ---
    let event_fd = Arc::new(eventfd(0, EfdFlags::EFD_NONBLOCK).map_err(|e| MainError::Epoll(e))?);
    ...
    for layer in layers.values() {
        ...
        for button in &layer.buttons {
            uinput
                .set_keybit(button.action)
                .map_err(|e| MainError::Input(std::io::Error::new(std::io::ErrorKind::Other, e)))?;
        }
        // Register buttons from split layout media section
        if let Some(split) = &layer.split {
            for button in &split.media {
                uinput.set_keybit(button.action).map_err(|e| {
                    MainError::Input(std::io::Error::new(std::io::ErrorKind::Other, e))
                })?;
            }
        }
    }
    ...
    loop {
        if cfg_mgr.update_config(&mut cfg, &mut layers, width) {
            ...
            active_layer = if current_session
                .as_ref()
        ...
        let any_changed = if let Some(split) = &get_active_layer(&layers, active_layer)?.split {
            ...
            get_active_layer(&layers, active_layer)?
                .buttons
                .iter()
                .any(|b| b.changed)
        };
        ...
        let browser_buttons_changed = app_ui_manager
            .browser_screen
            .buttons
            .iter()
            .any(|b| b.changed);
        let browser_buttons_active = app_ui_manager
            .browser_screen
            .buttons
            .iter()
            .any(|b| b.active);
        ...
        if needs_complete_redraw || any_changed || browser_buttons_changed {
            ...
            let app_layer3_slide_progress =
                if active_layer == LayerKey::Custom3 || last_layer == LayerKey::Custom3 {
            ...
            let clips = get_active_layer_mut(&mut layers, active_layer)?.draw(
                &cfg,
                width as i32,
                height as i32,
                &surface,
            ...
            if DEBUG_LOGGING && draw_time > std::time::Duration::from_millis(16) {
                println!(
                    "[main] SLOW DRAW: {:.2}ms (target: 16ms for 60fps)",
                    draw_time.as_millis() as f64
                );
            }
            ...
            match event.data() {
                ...
                3 => {
                    ...
                    if let Ok(new_state) = event_rx.try_recv() {
                        ...
                        if DEBUG_LOGGING {
                            println!(
                                "[main] Session changed: {} (current: {:?}, new: {:?})",
                                session_changed, current_session, new_state
                            );
                        }
                            ...
                            if new_state.is_logged_in {
                                ...
                                if let Some(stream) = helper_stream.take() {
                                    ...
                                    if let Err(e) = safe_epoll_delete(&epoll, &stream) {
                                        eprintln!(
                                            "[main] Failed to remove helper stream from epoll: {}",
                                            e
                                        );
                                    }
                                    helper_reader = None;
                ...
                4 => {
                    ...
                    if let Some(stream) = helper_manager.accept_connection() {
                        ...
                        if let Err(e) =
                            safe_epoll_add(&epoll, &stream, EpollEvent::new(EpollFlags::EPOLLIN, 5))
                        {
                        ...
                        if let Some(fd) = helper_listener_fd.take() {
                            ...
                            if let Err(e) = safe_epoll_delete(&epoll, &listener_fd_obj) {
                                eprintln!(
                                    "[main] Failed to remove helper listener from epoll: {}",
                                    e
                                );
                            }
                        }
                                ...
                                Ok(n) => {
                                    ...
                                    if let Ok(text) = std::str::from_utf8(data) {
                                        ...
                                        for part in text.split('\n') {
                                            ...
                                            if new_vlc_focused != vlc_window_focused {
                                                ...
                                                if vlc_window_focused {
                                                    ...
                                                    if let Some(user) = &current_user {
                                                        if let Some(fd) = vlc_helper_manager.start(
                                                            user,
                                                            current_session
                                                                .as_ref()
                                                                .and_then(|s| s.leader)
                                                            ...
                                                            if let Err(e) = safe_epoll_add(
                                                                ...
                                                                vlc_helper_listener_fd = Some(
                                                                    listener_fd_obj.into_raw_fd(),
                                                                );
                                                            }
                                                        }
                                                    } else {
                                                    ...
                                                    if let Some(stream) = vlc_helper_stream.take() {
                                                        if let Err(e) =
                                                            safe_epoll_delete(&epoll, &stream)
                                                        {
                                                    ...
                                                    if let Some(fd) = vlc_helper_listener_fd.take()
                                                    {
                                                        let listener_fd_obj =
                                                            unsafe { OwnedFd::from_raw_fd(fd) };
                                                        if let Err(e) = safe_epoll_delete(
                                                            &epoll,
                                                            &listener_fd_obj,
                                                        ) {
                                            ...
                                            let new_browser_focused = class_lower == "firefox"
                                                || class_lower == "chrome"
                                                || class_lower == "chromium"
                                                || class_lower == "brave"
                                                || class_lower == "brave-browser"
                                                || class_lower == "edge"
                                                || class_lower == "safari"
                                                || class_lower == "opera"
                                                || class_lower == "google-chrome";
                                            ...
                                            if new_browser_focused != browser_window_focused {
                                                ...
                                                if browser_window_focused && !new_browser_focused {
                                                    ...
                                                    if let Some(fd) =
                                                        browser_helper_listener_fd.take()
                                                    {
                                                        let listener_fd_obj =
                                                            unsafe { OwnedFd::from_raw_fd(fd) };
                                                        if let Err(e) = safe_epoll_delete(
                                                    ...
                                                    if let Some(user) = &current_user {
                                                            ...
                                                            )
                                                        {
                                                            ...
                                                            if let Err(e) = safe_epoll_add(
                                                                ...
                                                                browser_helper_listener_fd = Some(
                                                                    listener_fd_obj.into_raw_fd(),
                                                                );
                                                            }
                                                        }
                                            ...
                                            } else if new_browser_focused && browser_window_focused
                                            {
                                                ...
                                                if current_window_class.as_ref()
                    ...
                    if let Some(stream) = vlc_helper_manager.accept_connection() {
                        ...
                        if let Ok(stream_clone) = stream.try_clone() {
                            ...
                            if let Some(fd) = vlc_helper_listener_fd.take() {
                                ...
                                if let Err(e) = safe_epoll_delete(&epoll, &listener_fd_obj) {
                                    eprintln!(
                                        "[main] Failed to remove VLC listener from epoll: {}",
                                        e
                                    );
                                }
                            }
                            ...
                            match reader.get_mut().read(&mut buf) {
                                Ok(0) => {
                                    ...
                                    if let Some(stream) = vlc_helper_stream.take() {
                                        if let Err(e) = safe_epoll_delete(&epoll, &stream) {
                                            eprintln!(
                                                "[main] Failed to remove VLC stream from epoll: {}",
                                                e
                                            );
                                        }
                                    }
                                ...
                                Ok(n) => {
                                    ...
                                    if let Ok(text) = std::str::from_utf8(data) {
                                        for part in text.split('\n') {
                                            ...
                                            if let Ok(vlc_status) =
                                                serde_json::from_str::<serde_json::Value>(part)
                                            {
                                                // Update VLC screen with the status
                                                if let Some(is_playing) = vlc_status
                                                    .get("is_playing")
                                                    .and_then(|v| v.as_bool())
                                                {
                                                    if let Some(position) = vlc_status
                                                        .get("position")
                                                        .and_then(|v| v.as_f64())
                                                    {
                                                        if let Some(title) = vlc_status
                                                            .get("title")
                                                            .and_then(|v| v.as_str())
                                                        {
                                                            if let Some(artist) = vlc_status
                                                                .get("artist")
                                                                .and_then(|v| v.as_str())
                                                            {
                                                                if let Some(duration) = vlc_status
                                                                    .get("duration")
                                                                    .and_then(|v| v.as_i64())
                                                                {
                                                                    // Create a VlcStatus struct and update the VLC screen
                                                                    let status =
                                                                        crate::helper::VlcStatus {
                                                                            is_playing,
                                                                            position,
                                                                            duration,
                                                                            title: title
                    ...
                    if let Some(mut stream) = browser_helper_manager.accept_connection() {
                        ...
                        if let Some(window_class) = &current_window_class {
                            ...
                            if let Err(e) = stream.write_all(browser_type_msg.as_bytes()) {
                                ...
                                println!(
                                    "[main] Sent exact browser type '{}' to browser helper",
                                    window_class
                                );
                            }
                        }
                        ...
                        if let Err(e) =
                            safe_epoll_add(&epoll, &stream, EpollEvent::new(EpollFlags::EPOLLIN, 9))
                        {
                                ...
                                Ok(n) => {
                                    ...
                                    if let Ok(text) = std::str::from_utf8(data) {
                                        for part in text.split('\n') {
                                            ...
                                            if let Ok(browser_status) =
                                                serde_json::from_str::<serde_json::Value>(part)
                                            {
                                                // Update browser screen with the status
                                                if let Some(url) = browser_status
                                                    .get("url")
                                                    .and_then(|v| v.as_str())
                                                {
                                                    if let Some(title) = browser_status
                                                        .get("title")
                                                        .and_then(|v| v.as_str())
                                                    {
                                                        if let Some(can_go_back) = browser_status
                                                            .get("can_go_back")
                                                            .and_then(|v| v.as_bool())
                                                        {
                                                            if let Some(can_go_forward) =
                                                                browser_status
                                                                    .get("can_go_forward")
                                                                    .and_then(|v| v.as_bool())
                                                            {
                                                                if let Some(is_loading) =
                                                                    browser_status
                                                                        .get("is_loading")
                                                                        .and_then(|v| v.as_bool())
                                                                {
        ...
        for event in &mut input_tb.clone().chain(input_main.clone()) {
            ...
            match event {
                ...
                Event::Keyboard(KeyboardEvent::Key(key)) => {
                    ...
                    } else if key.key() == Key::Macro1 as u32
                        && key.key_state() == KeyState::Pressed
                    {
                        ...
                        needs_complete_redraw = true;
                    } else if key.key() == Key::Macro2 as u32
                        && key.key_state() == KeyState::Pressed
                    {
                        active_layer = LayerKey::Custom2;
                        needs_complete_redraw = true;
                    } else if key.key() == Key::Macro3 as u32
                        && key.key_state() == KeyState::Pressed
                    {
                ...
                Event::Touch(te) => {
                    ...
                    match te {
                        TouchEvent::Down(dn) => {
                            ...
                            if let Some((group, idx)) =
                                get_active_layer_mut(&mut layers, active_layer)?.hit_test(
                                    _x,
                                    width as i32,
                                    Some(active_layer.clone()),
                                )
                            {
                                match group {
                                    "modules" => {
                                        ...
                                        touches.insert(
                                            dn.seat_slot(),
                                            (active_layer.clone(), group, idx),
                                        );
                                        println!(
                                            "[main] Touch stored for modules group, slot: {}",
                                            dn.seat_slot()
                                        );
                                        ...
                                        if let Some(window_class) = &current_window_class {
                                            ...
                                            let total_width =
                                                (width as i32 - pixel_shift_width as i32) as f64;
                                            ...
                                            if let Some(app_action) = app_ui_manager
                                                .hit_test_app_ui(
                                                    adjusted_x,
                                                    adjusted_y,
                                                    modules_x,
                                            ...
                                            {
                                                println!(
                                                    "[main] App action detected: {:?}",
                                                    app_action
                                                );
                                                println!("[main] Checking browser condition for window_class: {}", window_class);
                                                if window_class_lc == "firefox"
                                                    || window_class_lc == "chrome"
                                                    || window_class_lc == "chromium"
                                                    || window_class_lc == "chromium-browser"
                                                    || window_class_lc == "brave"
                                                    || window_class_lc == "brave-browser"
                                                    || window_class_lc == "edge"
                                                    || window_class_lc == "safari"
                                                    || window_class_lc == "opera"
                                                    || window_class_lc == "google-chrome"
                                                {
                                                    ...
                                                    match app_action {
                                                        AppAction::Browser(BrowserAction::Back) => {
                                                            ...
                                                            println!(
                                                                "[main] Executing Browser Back"
                                                            );
                                                            app_ui_manager.browser_screen.buttons
                                                                [0]
                                                            .active = true;
                                                            app_ui_manager.browser_screen.buttons
                                                                [0]
                                                            ...
                                                            {
                                                                if let Err(e) = send_browser_command(
                                                                    stream, "back",
                                                                ) {
                                                        ...
                                                        AppAction::Browser(
                                                            ...
                                                            println!(
                                                                "[main] Executing Browser Forward"
                                                            );
                                                            app_ui_manager.browser_screen.buttons
                                                                [1]
                                                            .active = true;
                                                        ...
                                                        AppAction::Browser(
                                                            ...
                                                            println!(
                                                                "[main] Executing Browser Refresh"
                                                            );
                                                            app_ui_manager.browser_screen.buttons
                                                                [2]
                                                            .active = true;
                                                        ...
                                                        AppAction::Browser(
                                                            BrowserAction::BookmarksManager,
                                                        ) => {
                                                        ...
                                                        AppAction::Browser(
                                                            ...
                                                            println!(
                                                                "[main] Executing Browser New Tab"
                                                            );
                                                            app_ui_manager.browser_screen.buttons
                                                                [5]
                                                            .active = true;
                                                ...
                                                } else if window_class_lc == "vlc"
                                                    && vlc_helper_stream.is_some()
                                                    && vlc_window_focused
                                                {
                                                    ...
                                                    match app_action {
                                                        ...
                                                        AppAction::Vlc(VlcAction::DragHead(
                                                            position,
                                                        )) => {
                                                            ...
                                                            static mut LAST_SEEK_POSITION: f64 =
                                                                0.0;
                                                            unsafe {
                                                                if (position - LAST_SEEK_POSITION)
                                                                    .abs()
                                                                    > 0.01
                                                                {
                                    ...
                                    "media" => {
                                        if let Some(split) =
                                            &mut get_active_layer_mut(&mut layers, active_layer)?
                                                .split
                                        {
                        ...
                        TouchEvent::Motion(mtn) => {
                            ...
                            println!(
                                "[main] Motion event: group={}, idx={}, coords=({}, {})",
                                group, idx, _x, _y
                            );
                            match *group {
                                "modules" => {
                                    ...
                                    if let Some(window_class) = &current_window_class {
                                        let any_browser_button_active = app_ui_manager
                                            .browser_screen
                                            .buttons
                                            .iter()
                                            .any(|b| b.active);
                                        ...
                                        if let Some(app_action) = app_ui_manager.hit_test_app_ui(
                                            adjusted_x,
                                            adjusted_y,
                                            modules_x,
                                            modules_y,
                                            ...
                                            println!(
                                                "[main] Motion - App action detected: {:?}",
                                                app_action
                                            );
                                                ...
                                                && vlc_helper_stream.is_some()
                                            {
                                                ...
                                                println!(
                                                    "[main] No active touch interaction for motion"
                                                );
                                            }
                                        }
                                    }
                                ...
                                "media" => {
                                    if let Some(split) =
                                        &mut get_active_layer_mut(&mut layers, *layer)?.split
                                    {
                                        let button = &mut split.media[*idx];
                                        if button.action == Key::Unknown {
                                            continue;
                                ...
                                "flat" => {
                                    let button = &mut get_active_layer_mut(&mut layers, *layer)?
                                        .buttons[*idx];
                                    if button.action == Key::Unknown {
                                        continue;
                                    }
                                    ...
                                    if let Some(window_class) = &current_window_class {
                                            ...
                                            || window_class_lc == "google-chrome"
                                        {
                                            ...
                                            if any_browser_button_active {
                                                ...
                                                for button in
                                                    &mut app_ui_manager.browser_screen.buttons
                                                {
                                                    button.active = false;
                                                    button.changed = true;
                                                }
        ...
        if let Err(e) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            eprintln!(
                "[main] Error during VLC helper manager status check: {:?}",
                e
            );
        }
        if let Err(e) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            eprintln!(
                "[main] Error during browser helper manager status check: {:?}",
                e
            );
        }
        ...
        unsafe {
            ...
            if PROCESS_STATUS_COUNTER % 1000 == 0 {
                // Log every 1000 frames
                println!(
                    "[main] Process status - Main helper: {}, VLC helper: {}, Browser helper: {}",
        ...
        unsafe {
            ...
            if FORCE_CLEANUP_COUNTER % 10000 == 0 {
                // Every 10000 frames (reduced frequency)
                if DEBUG_LOGGING {
                ...
                if let Err(e) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    ...
                    eprintln!(
                        "[main] Error during browser helper manager cleanup: {:?}",
                        e
                    );
                }
            }
        ...
        if let Some(user) = &current_user {
            if helper_manager.is_process_none() && helper_manager.check_session_ready() {
                if DEBUG_LOGGING {
                    println!(
                        "[main] Session is now ready, starting main helper for user: {}",
                        user
                    );
                }
                if let Some(fd) = helper_manager.start(
                    user,
                    current_session.as_ref().and_then(|s| s.leader).unwrap_or(0),
                ) {
                    if DEBUG_LOGGING {
                    ...
                    if let Err(e) = safe_epoll_add(
                        &epoll,
                        &listener_fd_obj,
                        EpollEvent::new(EpollFlags::EPOLLIN, 4),
                    ) {
                    ...
                    println!(
                        "[main] ERROR: Failed to start main helper for user: {}",
                        user
                    );
                }
            }
